...
namespace BransItems.Modules.Pickups.Items.Essences
{
    class EOStrength : ItemBase
    {
		...
		public override string ItemPickupDesc => "Gain a flat damage buff";
		public override string ItemFullDescription => $"Gain a flat <style=cIsDamage>{DamageGain}</style> buff. <style=cStack>(+{DamageGain}).";
		...
		{
			DamageGain = config.Bind<float>("Item: " + ItemName, "Base damage given to character", 1f, "How much base damage should Essense of Strength grant?").Value;
